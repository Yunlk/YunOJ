...
type Stats struct {
	...
	Processing map[string]int64 `json:"processing"`
}
...
func (q *Queue) Stats(ctx context.Context) (Stats, error) {
	...
	result := Stats{Queued: queued, Processing: map[string]int64{}}
	var cursor uint64
	for {
		keys, next, err := q.rdb.Scan(ctx, cursor, processingPrefix+"*", 100).Result()
		if err != nil {
			return Stats{}, err
		}
		for _, key := range keys {
			...
			result.Processing[strings.TrimPrefix(key, processingPrefix)] = n
		}
		cursor = next
		if cursor == 0 {
			break
...
func processingKey(consumerID string) string {
	return processingPrefix + consumerID
}
...
func (q *Queue) Pop(ctx context.Context, consumerID string, block time.Duration) (int64, error) {
	val, err := q.rdb.BRPopLPush(ctx, mainKey, processingKey(consumerID), block).Result()
...
func (q *Queue) Done(ctx context.Context, consumerID string, submissionID int64) error {
	return q.rdb.LRem(ctx, processingKey(consumerID), 1, submissionID).Err()
}
...
func (q *Queue) PeekProcessing(ctx context.Context, consumerID string) ([]int64, error) {
	vals, err := q.rdb.LRange(ctx, processingKey(consumerID), 0, -1).Result()
	if err != nil {
		return nil, err
	}
...
func (q *Queue) Reclaim(ctx context.Context, consumerID string) error {
	return recoverScript.Run(ctx, q.rdb,
		[]string{processingKey(consumerID), mainKey}).Err()
}

// RemoveProcessingEverywhere 从所有消费者的处理中列表删除任务。
// 集群节点重建后消费者 ID 可能变化，恢复过期任务时不能只清理当前节点。
func (q *Queue) RemoveProcessingEverywhere(ctx context.Context, submissionID int64) error {
	var cursor uint64
	for {
		keys, next, err := q.rdb.Scan(ctx, cursor, processingPrefix+"*", 100).Result()
		if err != nil {
		...
		for _, key := range keys {
			if err := q.rdb.LRem(ctx, key, 0, submissionID).Err(); err != nil {
				return err
			}
		}
		cursor = next
