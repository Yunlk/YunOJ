...
func (a *API) validateSubmitBasics(r *http.Request, language, code string, userID int64) (string, int) {
	...
	enabled, err := a.store.IsJudgeLanguageEnabled(r.Context(), language)
	if err != nil {
		slogError(r, "读取语言状态", err)
		return "提交失败，请稍后重试", http.StatusInternalServerError
	}
	if !enabled {
		return "该语言当前已停用", http.StatusBadRequest
	}
...
// handleListSubmissions 提交列表。
// 普通用户只能查询自己的提交，管理员可使用题目/用户/状态条件查询全量记录。
func (a *API) handleListSubmissions(w http.ResponseWriter, r *http.Request) {
	u, loggedIn := userFromCtx(r.Context())
	if !loggedIn {
		// 路由已使用 requireAuth，这里保留防御性校验，避免处理器被其他路由复用时泄露数据。
	...
	if isAdmin {
		...
		// 不信任查询字符串中的 user_id，防止普通用户通过改 URL 查询他人提交。
		userID := u.ID
		f.UserID = &userID
	}
...
func (a *API) handleLanguages(w http.ResponseWriter, r *http.Request) {
	configs, err := a.store.ListJudgeLanguages(r.Context())
	if err != nil {
		slogError(r, "读取语言列表", err)
		writeError(w, http.StatusInternalServerError, "读取语言列表失败")
		return
	}
	...
	all := langs.PublicAll()
	items := make([]langs.PublicLanguage, 0, len(all))
	for _, language := range all {
		if enabled[language.Key] {
