        ...
        <tbody>
          {loading ? (
            ...
            problems.map((p) => (
              <tr key={p.id}>
                <td
                  className={`mono problem-list-id-cell ${p.my_status === 'accepted' ? 'accepted' : p.my_status === 'wrong_answer' ? 'wrong-answer' : ''}`}
                  aria-label={`题目 ${p.id}${p.my_status === 'accepted' ? '，AC 已通过' : p.my_status === 'wrong_answer' ? '，WA 尚未通过' : ''}`}
                >
                  {p.id}
                </td>
                <td>
